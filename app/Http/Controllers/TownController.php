...
class TownController extends Controller
{
    ...
    public function Update(Request $request)
    {
        ...
        $db = DB::table('town')->where('town_name', $townName)->where('town_id', '!=', $townId)->count();
        if ($db > 0) {
            return response()->json(['message' => 'Town name already present please enter a new Town Name'], 200);
        } else {
    ...
    public function GetAdjacentTownWithoutPagination(Request $request)
    {
        ...
        ->join('province','province.province_id','province.province_id')
    ...
    // ->where(function ($q) use ($townName) {
    //     if($townName!=null){
    //     $q ->where('town.town_name',$townName );
    //         }
    //          })
}
