...
return response()->json(['resultData' => $getQuery], 200);
    ...
    public function Save(Request $request)
    {
        ...
        $barangay= DB::table('barangay')->where('barangay_name',$barangay_name)->count();
        if ( $barangay>0) {
    ...
    public function Update(Request $request)
    {
        ...
        $db = DB::table('barangay')->where('barangay_name', $barangay_name)->where('barangay_id', '!=',$barangay_id )->count();
        if ($db > 0) {
            return response()->json(['message' => 'Barangay name already present please enter a new Barangay Name'], 200);
        }
        else {
